...
foreach (var key in allKeys)
{
   ...
   var (paramCount, sequential) = AnalyzeStringParameters(defaultValue);
    ...
    {
        // No parameters
        parametersList = "ulong? guildId = null";
        argumentsList = "Array.Empty<object>()";
    }
    else if (sequential)
    {
        // Sequential parameters
        parametersList = $"ulong? guildId = null, {string.Join(", ", Enumerable.Range(0, paramCount).Select(i => $"object param{i}"))}";
        argumentsList = $"new object[] {{ {string.Join(", ", Enumerable.Range(0, paramCount).Select(i => $"param{i}"))} }}";
    }
    else
    {
        // Non-sequential parameters
        parametersList = "ulong? guildId = null, params object[] data";
        argumentsList = "data";
    }
